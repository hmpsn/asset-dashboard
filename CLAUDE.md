...
**F3/F4 net-new primitives (design-system rebuild, `@ds-rebuilt`):** `Drawer` (overlay/), `Avatar`, `IntentTag`, `DataTable`, `MetricTile`, `Sparkline`, `Meter`, `KeyValueRow` (+`DefinitionList`), `BoardColumn` (+`BoardCard`), `Segmented` (forms/), `LensSwitcher` (forms/), `FilterChip` (forms/), `SearchField` (forms/), `RadioGroup` (forms/), `AppShell` (layout/), `PageContainer` (layout/), `Toolbar` (+`ToolbarSpacer`, layout/), `GroupBlock` (layout/), `NavItem` (layout/), `NavGroup` (layout/) — all exported from the `src/components/ui/` barrels. Shared machinery: `useRovingTabindex` (the keyboard-nav bar hook for Segmented/LensSwitcher/RadioGroup/Toolbar/DataTable rows and the rebuilt shell nav) and `ui/overlay/overlayUtils.ts` (`getFocusable` + shared scroll-lock — the ONLY sanctioned focus/keyboard-trap source for DS overlays; never hand-roll one). `IntentTag`'s `INTENT_TONE` map is the canonical keyword-intent→hue mapping. Mutation feedback stays on the existing `src/components/Toast.tsx` `useToast` (do NOT build a second Toast).
...
- **Model IDs**: never hard-code an LLM model-ID string literal (`model: 'gpt-…'` / `'claude-…'`) at a `server/` or `scripts/` call site. Import a semantic role from `server/model-manifest.ts` (`MODEL_ROLES.*`, `DEFAULT_ANTHROPIC_MODEL`, `DEFAULT_OPENAI_MODEL`) so the next model upgrade is a one-file edit. The manifest also owns the single pricing table and per-family request-param rules. Full contract: [docs/rules/model-manifest.md](./docs/rules/model-manifest.md). Enforced by pr-check (`Inline model-ID literal in server code` / `in scripts`; hatch `// model-id-ok` for genuinely manifest-internal literals).
...
- **Prompt assembly layers must not duplicate content** — `buildSystemPrompt` in `server/prompt-assembly.ts` injects voice DNA + guardrails into the system message when `profile.status === 'calibrated'` (Layer 2). Any user-prompt code that manually inlines the same DNA must guard on `profile.status !== 'calibrated'` to avoid redundant injection that wastes tokens and confuses the model. Use `buildVoiceCalibrationContext(profile)` from `server/voice-calibration.ts` rather than hand-rolling the guard inline. Under `content-generation-context-v2`, build one context per logical run and reuse its captured prompt authority through `buildSystemPromptFromAuthority()`; do not re-read or re-render voice in each stage. User voice examples and system DNA/guardrails each appear exactly once per effective prompt, and external scraped text stays in timestamped untrusted wrappers.
...
- **AI freshness fingerprints must cover the exact effective inputs, and automatic refresh must preserve operator edits** — hash the canonical system prompt + user prompt (or one canonical object that renders them) after authority resolution, not a convenient subset of source ids. A force-refresh nonce may bypass cache but must not become the persisted canonical fingerprint. If an operator can edit saved AI output, background/automatic generation exposes stale/refresh-available state and never replaces the edit; only an explicit replacement action may do so. Guard in-flight generation with a version-conditional save. See `docs/rules/post-parity-owner-closure.md` for the Strategy POV reference implementation.
- **Generation runtime integrity is an end-to-end contract.** Required-stage failures must never be reported as successful artifacts; durable AI output saves are version-conditional; completed-response caching is an explicit named-operation policy; authoritative evidence fields survive every adapter; dependent recomputes are ordered; and durable artifacts retain run provenance. Correctness repairs ship unflagged, while intentional output/ranking changes use one narrow server-side rollout flag with measured exit criteria. See `docs/rules/generation-runtime-integrity.md`.
- **Structured generated-page output is typed authority, not a prompt suggestion.** Per-block heading locks, semantic rendering, and required internal links must survive sanitization, revision, persistence, and final audit. Internal anchors come only from a workspace-census-backed allowlist frozen during preflight; zero/self/unknown links fail closed. Automatic prose revision uses the homogeneous accepted prose provider/model or preserves the draft in an attention state. See `docs/rules/generation-runtime-integrity.md` §9.
- **Webflow SEO copy uses canonical named operations.** Title/meta variations and per-page SEO copy sets route through the shared `seo-health` generation service and the model manifest's creative-writer role. Adapters may gather different authorized evidence, persist existing suggestion records, or report job progress, but must not duplicate prompts/parsers or let generation apply copy. Concrete specificity is evidence-conditional and internal links are limited to the caller's verified allowlist. See `docs/rules/seo-copy-operations.md`.
...
- **Approved-copy benchmarks are private, read-only, and blinded** — raw approved client copy, prompts, evidence, workspace identity, URLs, and reviewer identity stay in gitignored local artifacts. Tracked benchmark data is limited to typed contracts, synthetic fixtures, content hashes, rubric versions, and owner-approved aggregates. Benchmark tooling must not save, approve, send, publish, sync databases, or trigger paid generation by default. See `docs/rules/content-quality-benchmark.md`.
...
- **MCP-driven durable generation batches** — address source rows by durable IDs + expected revisions, run deterministic preflight before paid work, and persist normalized domain run/item ledgers with resource-scoped idempotency, bounded cost/concurrency, cancellation/resume checkpoints, version-conditional artifact commits, and truthful partial outcomes. A generic job result is summary-only; generated content is at most `ready_for_human_review` and never auto-approved/sent/published. Missing facts stay typed `[NEEDS CLIENT INPUT: …]` requirements, matrix dimension labels are not evidence, and brand/client review reuses the unified deliverable spine. See [mcp-deliverable-generation.md](./docs/rules/mcp-deliverable-generation.md).
- **MCP server profiles are enforced boundaries, not presentation hints.** The full `/mcp` profile is the backward-compatible authority. Additive profiles such as `/mcp/operator` must derive discovery and dispatch from one canonical allowlist, return `not_found` for hidden-tool invocation, and preserve credential/workspace authorization as a separate mandatory check. Compact discovery may remove explanatory schema metadata but must retain every validation keyword, property, required field, and input constraint. See [mcp-operator-control-plane.md](./docs/rules/mcp-operator-control-plane.md).
- **Restricted MCP credentials are transport-profile-bound.** A credential minted for a restricted surface such as `/mcp/client` must be rejected at every broader MCP endpoint; a discovery allowlist alone is not authorization. Client-profile schemas omit workspace identity and the server injects the authenticated credential scope only after rejecting caller-supplied aliases. Every client tool is explicitly censused as bounded/read-only and hidden invocation returns generic `not_found`. See [mcp-client-readonly-profile.md](./docs/rules/mcp-client-readonly-profile.md).
...
- **UI Rebuild conventions** — files carrying the `@ds-rebuilt` marker opt into strict rebuild gates (tokens-only styling, `var(--dur-*)` motion, icons via the `<Icon>` component — no emoji-as-icon, no raw hex/palette classes, error severity). Full contract: [docs/rules/ui-rebuild-consistency.md](./docs/rules/ui-rebuild-consistency.md). Every quick-win trade-off shipped in a PR adds a `DEF-*` row to `data/ui-rebuild-deferred-ledger.json` in the same PR (`npm run verify:deferred-ledger` enforces schema, expiry, and roadmap links). The F4 rebuilt shell (`ui-rebuild-shell`) is additive and pilot-mounted: do not replace live `App.tsx`/`Sidebar.tsx`/`Breadcrumbs.tsx` until a pilot phase explicitly opts in.
...
| `docs/rules/generation-runtime-integrity.md` | Truthful completion, version-conditional saves, cache policy, evidence survival, ordered recompute, provenance, and rollout contracts |
