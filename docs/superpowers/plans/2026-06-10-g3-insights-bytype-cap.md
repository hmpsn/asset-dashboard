...
| ↳ `server/monthly-digest.ts:69` | via helper — **and** a pinned contract test (`tests/unit/monthly-digest.test.ts` "uses full slice rollups for resolved items beyond the prompt-facing top-100 cap") requires FULL coverage for the deterministic wins/resolved rollups (resolved/positive insights are typically low-impact and fall outside top-100) | documented direct `getInsights(ws.id)` read with `// intel-builder-ok` hatch — full iteration is no longer slice-backed post-cap (per intelligence-consumer-builders.md exception rule). The AI prompt path keeps using `buildRecommendationGenerationContext`. |
| ↳ `server/admin-chat-context.ts:619` | via helper — verified prompt-only (`buildInsightsContext` top-10 sections, no count reporting) | no file change; top-100 prompt bound is the intent |
...
| `server/routes/client-intelligence.ts:42` `summarizeInsightsForClient` | flattens uncapped byType for filtered totals | derive counts from the new `countsByTypeBySeverity` pre-cap type×severity matrix. Two pinned contracts must hold simultaneously: (1) joint exclusion of admin-only types AND positive severity (`tests/integration/client-intelligence-endpoint.test.ts:564`, `tests/contract/client-intelligence-tiers.test.ts`), and (2) exact totals beyond the `all` 100-cap (`client-intelligence-endpoint.test.ts:608` pins total=105 on a 105-insight workspace — caught by the full-suite pre-commit hook, not the byType grep). Neither `insights.all` filtering (caps at 100) nor `countsByType`+`bySeverity` (no joint distribution) satisfies both, so the slice carries the matrix. |
