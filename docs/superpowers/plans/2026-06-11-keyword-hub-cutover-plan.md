...
Delete: `KeywordCommandCenter.tsx`, `RankTracker.tsx`, `KeywordRow.tsx`, `KeywordSparkline.tsx`, `VariantSubRow.tsx`, `KeywordDetailPanel.tsx` (orphan); tests `KeywordCommandCenter.test.tsx`, `KeywordCommandCenterDeepLink.test.tsx`, `KeywordActionMenu.b1.test.tsx`, `RankTracker.test.tsx`; App.tsx lazy imports + flag conditionals (:435-436 → unconditional Hub; :452 RankTracker case; flag read :260 if unused after); flag-OFF fallbacks in `WorkspaceHome.tsx:65`, `RankingsSnapshot.tsx:27`, `MeetingBriefPage.tsx:59`; rename `KeywordCommandCenter-ws-invalidation.test.tsx` → reflects registry contract; update `rank-tracking-key-collapse.test.ts:66-68`. **KEEP:** seo-ranks redirect (until D), `SummaryMetric.tsx` (adopted by KeywordHub in Phase A — do NOT delete), shared drawer/menu/bulk components + helpers, all queryKeys/hooks/wsInvalidation (Hub uses them), statusConfig domain.
