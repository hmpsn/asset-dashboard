...
2. **🚫 BLOCKING — F2b consistency gates must ship FIRST.** Verified at HEAD: `lint:ds-adherence` and `verify:bundle-budget` **do not exist in `package.json`** — Task 9.3 lists them as hard DoD gates, so the pilot is **not completable** until an F2b PR ships that toolchain (the `lint:ds-adherence` ESLint-wrapper lane, the theme×state snapshot matrix, `verify:bundle-budget`). **F2b is the immediate next executable step, before this pilot.** Until it lands, drop those two commands from Task 9.3's gate list (they'll `missing script`-fail) and treat the snapshot/bundle DoD boxes as REVIEW, not AUTO.
...
- [ ] **8.5** Gate-seeding (F2b): add the Keywords surface's snapshot cells (surface × {dark, light} × {empty, loading, error, locked, populated}); confirm `lint:ds-adherence` + `verify:bundle-budget` pass on the surface. Commit.
...
- [ ] **9.3** Full gates, sequential: `npm run typecheck && npx vite build && npx tsx scripts/pr-check.ts && npm run lint:hooks && npm run lint:ds-adherence && npm run verify:feature-flags && npm run verify:deferred-ledger && npm run verify:coverage-ratchet && npx vitest run` (FULL suite — the KCC contract tests + nav/tab-deep-link tests must stay green).
...
- [ ] Deferrals ledgered (inline sparkline, avg-pos/Δ KPIs); F2b gates seeded + green
