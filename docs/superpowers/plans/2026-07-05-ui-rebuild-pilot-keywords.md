...
2. **🚫 BLOCKING — the F2b gates PR must merge FIRST.** F2b (`docs/superpowers/plans/2026-07-05-ui-rebuild-f2b-gates.md`) ships the two CI-native gates this pilot's DoD uses: **`verify:bundle-budget`** and **`vitest-axe`** (`expectNoA11yViolations`). `lint:ds-adherence` was retired (redundant — tsc + the F2a `ds-*` rules cover it). Automated **visual** regression is NOT an F2b gate — this pilot builds it (Task 8.6, Playwright Component Testing) and is its first proving ground. Until F2b merges, the pilot must not start; sequence F2b → pilot. ✅ **CLEARED — F2b merged to `staging` (PR #1479, merge `6391fe5b0`, 2026-07-05); both gates live.** Three as-merged deltas to honor: **(a)** baseline seeding is `npm run verify:bundle-budget -- --update` — the `--` is REQUIRED to pass the flag through npm; **(b)** the hardened gate now **FAILS** (not warns) on any new chunk over a 50 KiB gzip cap and enforces an aggregate total-gzip ceiling, so the Task 8.5b `--update` is **mandatory** (it regenerates the baseline, which now carries a `total` field); **(c)** the a11y floor is enforced by `tests/contract/ds-rebuilt-a11y-coverage.test.ts` — **EVERY** new `@ds-rebuilt` component with a test must call `expectNoA11yViolations` or the contract lane fails; keep its `KNOWN_A11Y_GAPS` empty.
...
- [ ] **4.1** ⚠ **Reconcile F4 first (verified at HEAD 86e2d0c2a):** `RebuiltAppChrome` is a **COMPLETE app frame** — it renders its own `AppShell` + `RebuiltSidebar` + topbar + breadcrumb + `PageContainer`, and requires **11 props**: `workspaces, selected, tab, theme, pendingContentRequests, onCreate, onDelete, onLinkSite, onUnlinkSite, toggleTheme, onLogout` + `children`. `useRebuildShellEnabled()` reads `ui-rebuild-shell`. Re-grep the merged file to confirm before 4.2. ✅ Re-confirmed live post-F2b-merge via the `/__ds-harness` shell smoke: `RebuiltAppChrome` unconditionally renders `AppShell`+`RebuiltSidebar`+`RebuiltBreadcrumb`, the 11-prop API + `useRebuildShellEnabled()` hold, FA Sharp font paints, and needs-site gating + roving keyboard work.
...
- [ ] **8.5** Gate-seeding: (a) a11y — call `expectNoA11yViolations` (F2b's `vitest-axe` helper, `tests/component/a11y.ts`) on the KeywordsSurface + Drawer + each lens render in their component tests; fix any real violation. **The `ds-rebuilt-a11y-coverage` ratchet (contract lane) enforces this on EVERY new `@ds-rebuilt` component that has a test — not only these; add the assertion to each and keep `KNOWN_A11Y_GAPS` empty.** (b) bundle — `npx vite build --manifest && npm run verify:bundle-budget -- --update` (the `--` is REQUIRED to pass `--update` through npm) to add the Keywords bundle asset baseline entry — **mandatory**: the hardened gate FAILS on new chunks over the 50 KiB gzip cap, not warns. Commit the regenerated baseline (it gains a `total` field). Commit.
