...
> ⚠️ **DEFERRED (decided 2026-04-29).** This plan is not the next implementation step. After Pillar 2 shipped, a fresh-eyes audit against the live hmpsn.studio surface revealed that the highest-leverage next work is **field expansion** (areaServed on Service, knowsAbout on Organization, keywords on Article — all using workspace data we already have) rather than regression hardening. The new sequence is: (1) Pillar 2.1 SearchAction correctness fix, (2) Pillar 1 validator-bar, (3) `schema-yoast-parity-fields` plan + execution, then (4) reactivate this Pillar 3 plan. Pillar 3 is reactivated only AFTER the parity work settles, because adding compile-time `schema-dts` types now would have to be rewritten when templates expand. See `data/roadmap.json` entry `schema-pillar-3-gates` (status: deferred). The plan content below remains accurate for the work it describes; only the timing is changed.
