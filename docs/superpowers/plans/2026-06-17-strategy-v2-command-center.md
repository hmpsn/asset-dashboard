...
- **KEEP (do NOT delete) the reusable leaves** — `DecisionQueue`, `OpportunitiesList`, `DecayingPagesCard`, `LostQueryRecoveryCard`, `CannibalizationTriage`, `RankingDistribution`, `ContentGaps`, `TopicClusters`, `KeywordGaps`, `BacklinkProfile`, `CompetitiveIntel`, `AuthorityAndBacklinks`. Several become **temporarily orphaned** (zero importers) until Phases 1–5 re-home them — add an orphan-tracker comment to `index.ts` naming the v2 phase that re-imports each.
