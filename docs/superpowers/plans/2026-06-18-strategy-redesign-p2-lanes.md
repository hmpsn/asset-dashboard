...
- `StrategyDiff` (`realLeaves.strategyDiff`) and `intelligenceSignalsEl` remain in Overview — do NOT move them. `intelligenceSignalsEl` appears in the flag-ON Overview immediately after `realLeaves.cannibalization`; it stays there until P4's signal-fold owns it. Do NOT move `intelligenceSignalsEl` to the tab.

**Flag-OFF correctness check (critical):** with `strategy-command-center = false` (the flag-OFF path), **flag-OFF renders byte-identical** — the 3 keyword surfaces (`SiteTargetKeywords`, `KeywordOpportunities`, `clientFeedbackCombinedEl`) stay on the Overview inside the "Reference & Analysis" block, the tab label remains "Rankings" (not "Keywords & Rankings"), and `intelligenceSignalsEl` stays on the Overview. The IA move (rename + move the 3 keyword surfaces to the "Keywords & Rankings" tab) is **flag-gated** — applied only in the flag-ON path. The public data output is unchanged between flag states.
