...
**AMENDED during execution (Sol NEEDS_CONTEXT correct — verified 2026-07-17):** the "extend S&T to full parity" half is already DONE and must NOT be re-implemented. Both rebuilt lenses mount the *exact* legacy `AnomalyAlerts` component directly (`SearchLens.tsx:449`, `TrafficLens.tsx:332` both `import { AnomalyAlerts } from '../AnomalyAlerts'`), which already renders every anomaly class, severity chips, AI narratives, expandable detail, and persistent dismiss via `/api/anomalies/:id/dismiss`. Forking it into a rebuilt-only copy to "restore parity" would violate the campaign's port-don't-fork rule and create drift. So contracts (1) full-alert-class home in S&T and (2) dismissal round-trip are **already satisfied by the shared mount** — no change. The task narrows to the two genuinely-missing gaps below.
**Files (narrowed):** `cockpit-rebuilt` risk-stream — add an anomaly count/pointer (there is none today) + `server/domains/work-queue.ts` — fix the churn-signal classifier so positive-`severity` signals stop being admitted to Risk. Do NOT touch `search-traffic-rebuilt` or `AnomalyAlerts` (parity already met). **Required reading:** rebuild audit B4 + cockpit surface-audit churn findings.
**Contract (narrowed):** Cockpit shows "N anomalies" linking to `analytics-hub` with the anomalies section focused (two-halves deep-link: sender appends the section param, receiver reads it via useSearchParams); that Cockpit count shares ONE source with the S&T anomalies count (rate-display rule — never a separately-computed count); positive-severity signals stop rendering under "Risk".
**Tests:** Cockpit anomaly count == S&T source count (shared source); the two-halves deep-link lands on the focused anomalies section; a positive-severity churn signal is excluded from the Risk stream. (S&T parity is unchanged and stays covered by the existing AnomalyAlerts tests.)
