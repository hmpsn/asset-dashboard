...
>
> **Migrations reserved by this plan:** 070 (competitor_snapshots), 071 (competitor_alerts). Migrations 067-069 are already in use (067: discovery rawcontent size cap, 068: discovery rawcontent bytecount fix, 069: voice calibration feedback).
...
## Pre-Plan Audit — Current State (2026-04-23, re-verified)

Verified against HEAD. All line anchors use string patterns to survive minor line drift.

| Item | Key finding |
|------|------------|
| Item 8 | `ContentGap` in `shared/types/workspace.ts:67` — no `opportunityScore` field yet. All formula inputs (`volume`, `difficulty`, `trendDirection`, `impressions`) already exist on `ContentGap`. Enrichment block for content gaps ends at the SERP targeting section (anchor: `if (recs.length > 0) cg.serpTargeting = recs;`) in `server/routes/keyword-strategy.ts`. `computeOpportunityScore` helper inserts after the SERP targeting closing brace, before `// ── Cannibalization Detection`. |
| Item 9 | `InsightType` union in `shared/types/analytics.ts:189-203` — no `emerging_keyword` yet. `analytics-intelligence.ts` calls `getConfiguredProvider` and uses it in Phase 3B (lines 1252-1283). `trendDirection` is NOT imported — new import needed. Phase 5 (emerging keywords) inserts after Phase 3B closer brace, before Phase 3C (conversion attribution). `normQueryPageData`, `enrichAndUpsert`, `deleteStaleInsightsByType`, and `cycleStart` all confirmed in scope. `DomainKeyword` interface has both `trend?: number[]` and `url?: string`. `apiCache.wrap` signature: `wrap<T>(workspaceId, functionName, params, fn)`. |
| Item 10 | Latest migration is **069** (`069-voice-calibration-feedback.sql`). `competitor_snapshots` → migration **070**, `competitor_alerts` → migration **071**. `getConfiguredProvider` is NOT currently imported in `intelligence-crons.ts` — must be added. `intelligence-crons.ts` exports `startIntelligenceCrons()` and `stopIntelligenceCrons()`; called from `server/startup.ts` line 30. Competitor alert cards must be added to the `InsightCards` component grid — NOT to `InsightsDigest.tsx` (separate component, no shared imports). |
| Item 11 | No `schema_snapshots` table in the DB. Content freshness uses `PageKeywordMap.analysisGeneratedAt` from `page-keywords.ts` as the date proxy — threshold: 90 days old + ≥100 impressions. `listPageKeywords` returns `analysisGeneratedAt`, `impressions`, and `clicks`. `getInsights` is already imported in `recommendations.ts`. `RecPriority`, `recs`, `now`, `assignedTo` all confirmed in scope for freshness rec insertion before `const summary = {`. |
...
**Files:**
- Create: `server/db/migrations/070-competitor-snapshots.sql`
- Create: `server/db/migrations/071-competitor-alerts.sql`
...
```sql
-- server/db/migrations/070-competitor-snapshots.sql
...
- [ ] **Step 2: Create migration 071**

```sql
-- server/db/migrations/071-competitor-alerts.sql
...
Create a `CompetitorAlertCard` functional component in `InsightCards.tsx` following the existing card pattern (see `SiteHealthCard` for structure). Use `text-blue-400` for data values, `text-zinc-100` for headlines, `text-zinc-400` for body text. No purple. Add it to the grid inside the main `InsightCards` export function (the 6-card grid at the bottom of the file). Do NOT add to `InsightsDigest.tsx` — that component is for local client intelligence and does not share imports with `InsightCards.tsx`.
...
describe('content freshness detection', () => {
  ...
  it('GET /api/workspace/:id/insights returns freshness_alert for stale pages', async () => {
    ...
    const res = await ctx.get(`/api/workspace/${seed.workspaceId}/insights`);
