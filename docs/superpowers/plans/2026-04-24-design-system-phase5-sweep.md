...
| 5 | `no-rounded-literal` | `rounded-(sm\|md\|lg\|xl\|2xl\|3xl\|full)` | `src/components/ui/**`, `public/styleguide.*` | error | Use a concrete radius token class such as `rounded-[var(--radius-lg)]` or `rounded-[var(--radius-signature-lg)]` for asymmetric card signatures. |
