...
> **Dropped during verification:** the originally-proposed migration of `server/copy-generation.ts:116` from `callAnthropic` to `callAI()`. CLAUDE.md explicitly permits `callAnthropic` *"for creative prose"*, and copy generation is exactly that. See audit artifact "Corrections Applied During Verification" section.
