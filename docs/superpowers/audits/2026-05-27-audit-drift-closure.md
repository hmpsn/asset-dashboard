...
| Migrate `server/copy-generation.ts:116` from `callAnthropic` to `callAI()` | The line cited is a **stale comment** (`// Parse response — callAnthropic returns { text, ... }`). The actual AI call at line 106 already uses `callAI()`. Re-verifying the full repo: **zero** direct `callOpenAI`/`callAnthropic` callers outside `server/ai.ts` itself; the unified dispatcher is fully adopted. The original CLAUDE.md line that permitted direct `callAnthropic` "for creative prose" was itself stale and was updated in the same closure. | **Rescoped.** A new Plan C item replaces the stale comment and adds a pr-check rule blocking future direct imports of `callOpenAI`/`callAnthropic` outside `server/ai.ts`. |
