...
| `keyword-strategy-enrichment.ts:611` (content_gap inline) | inline 4-bucket coercion | `deriveValueIntent(cg.targetKeyword, cg.intent)` | **changes weight** — `StrategyContentGap.intent` is free-form (see below) |

- **Retire `toOpportunityIntent`** (defined `recommendations.ts:245`) once `:1482`/`:1539` are migrated.
- **`recommendations.ts:1420` is value-inert** — its `cg` comes from `listContentGaps`, whose `rowToContentGap` mapper (`content-gaps.ts:53`) coerces any non-4-bucket DB value (incl. `comparison`) to `'informational'` on read, so `deriveValueIntent` always takes the provided branch and the result equals `cg.intent`. Migrating it is single-classifier consistency with **zero behavior change**.
- **`keyword-strategy-enrichment.ts:611` CHANGES WEIGHT** — here `cg` is a freshly AI-synthesized `StrategyContentGap` whose `intent?: string` is **free-form** (`keyword-strategy-ai-synthesis.ts`), so `comparison`/absent/non-4-bucket labels DO occur. Under full-derive they reclassify (e.g. `comparison`→commercial 0.7 vs the old `null`→0.5), changing `cg.opportunityScore`. This runs inside the **canonical** `relaxConservatism=true` path (`keyword-strategy-generation.ts:427`, not the dark flag), so it shifts content-gap scores **on every strategy generation** — the intended full-derive correction, on a third surface. **Must be tested** (assert the new value), not assumed inert.
- So **three** sites change weight (`:1482`, `:1539`, enrichment `:611`); only `recommendations.ts:1420` is truly inert.
...
**Behavior-scope — DECIDED: full-derive (owner, 2026-06-05).** Recs classify intent the **same way the Hub does** (`comparison→commercial` **and** a regex fallback from the keyword when `searchIntent`/intent is absent), instead of defaulting absent/`comparison` to 0.5 — the true "one source of truth." Blast radius is bounded and correctness-positive, on **two** surfaces: (a) `ranking_opp` recs whose `searchIntent` is `comparison`/missing (`:1482`/`:1539`), and (b) content-gap `opportunityScore` for AI-synthesized gaps whose free-form intent is `comparison`/non-4-bucket (enrichment `:611`, on every strategy generation). Both manifest on the next strategy generation/regen, verified on staging. (The coerce-only alternative — fix `comparison` only, keep 0.5 for absent — was considered and rejected.)
...
- **OV-output parity (refactor safety):** every `computeOpportunityValue` branch yields a byte-identical **full `OpportunityScore` + `topOpportunityRationale`** pre/post for non-`comparison`, intent-present inputs; the `comparison`/absent-`searchIntent` cases assert the *new* intended values (incl. the intent component's evidence). `recommendations.ts:1420` asserts **zero** change (truly inert). **`enrichment.ts:611` asserts the *new* value** — drive the enrichment path with a `StrategyContentGap` whose free-form intent is `comparison` (and absent) and assert the content-gap `opportunityScore` reflects the commercial/regex-derived classification, not the old 0.5 default.
...
- **Drift hazard if partially migrated:** the weight-changing sites are `recommendations.ts:1482`/`:1539` AND `keyword-strategy-enrichment.ts:611` (free-form `StrategyContentGap.intent`) — leaving any un-migrated re-opens the disagreement. Only `recommendations.ts:1420` is truly inert. All four migrate + `toOpportunityIntent` retires **in one commit** (CLAUDE.md cross-cutting-constraint rule). NOTE the file is `server/keyword-strategy-enrichment.ts` (not the unrelated `server/insight-enrichment.ts`).
...
- **Blast radius is real but bounded + reversible:** the output changes are (a) `ranking_opp` rec intent weight for `comparison`/absent-`searchIntent` keywords, and (b) content-gap `opportunityScore` for AI-synthesized gaps with `comparison`/non-4-bucket intent — both manifest on the next strategy generation/regen, verified on staging before `staging→main`.
