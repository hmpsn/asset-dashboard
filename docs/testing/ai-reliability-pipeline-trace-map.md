...
| `seo-editor-assist` | `seo-health` | `server/routes/rewrite-chat.ts`, `webflow-seo-page-tools.ts`, `webflow-seo-rewrite.ts`, `webflow-seo-bulk-rewrite.ts`, `webflow-seo-jobs.ts`, `webflow-seo-apply.ts` | live-editor rewrite path plus `domains/seo-health/seo-copy-generation.ts` named operations (`seo-metadata-variations`, `seo-page-copy-set`) using page-assist intelligence, layered voice authority, strict Zod output, and verified-link filtering | generation itself is pure; adapters return transient copy or persist `seo_suggestions`; separate human apply paths own Webflow writes, activity, and broadcasts | `WS_EVENTS.PAGE_STATE_UPDATED`, `WS_EVENTS.STRATEGY_UPDATED`, bulk operation events; seo editor/strategy/workspace-home invalidations | `tests/unit/seo-copy-generation-service.test.ts`, `tests/unit/webflow-seo-bulk-rewrite-job.test.ts`, `tests/contract/seo-copy-operation-adapters.test.ts`, `tests/integration/webflow-seo-writes.test.ts`, `tests/contract/external-provider-write-failure-contract.test.ts` |
