...
## 3. Update BRAND_DESIGN_LANGUAGE.md (if UI changed)
...
- Verify colors follow the **Four Laws**: teal for actions, blue for data, emerald for success, purple for admin AI only
...
## 6. Update AI_CHATBOT_ROADMAP.md (if applicable)
...
## 7. Update feature-integration.md cross-link table (if applicable)

If the shipped feature adds new cross-links between tools, update the table in `docs/workflows/feature-integration.md` section 1.
...
- [ ] No violet/indigo colors introduced (grep for `violet\|indigo` in `src/components/`)
