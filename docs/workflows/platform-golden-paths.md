...
- For repeated workspace-scoped write lifecycles, prefer `runWorkspaceMutation()` (`server/workspace-mutation-helper.ts`) to keep read-before-write, transaction, mapped error, activity, and broadcast steps consistent.
