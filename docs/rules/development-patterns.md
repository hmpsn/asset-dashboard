...
// For endpoints that may 404 legitimately
const data = await getOptional<MyType>(`/api/maybe-exists/${id}`);

// For endpoints where errors should return null instead of throwing
const data = await getSafe<MyType>(`/api/safe/${id}`);
```
...
└── NO → Is this a public/client route (/api/public/...)?
    ...
    │   └── Use requireClientPortalAuth() or verifyClientToken()
    │
...
const ctx = createTestContext(13317); // Next available port
