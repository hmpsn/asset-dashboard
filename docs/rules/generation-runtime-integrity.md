...
- Every bounded provider dispatch identifies its exact model. Preview and runtime reservations derive provider-request output ceilings—including adaptive-thinking headroom—and pricing from the canonical model manifest before paid work; content-token limits alone are not a truthful spend envelope.
