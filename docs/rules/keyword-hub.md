...
- `/initial` is a first-mount transport optimization, not an interaction endpoint. The initial hook seeds the canonical summary key and the exact first rows key. Search, filter, sort, lens, and pagination changes use `/rows` with React Query previous-data retention; they must not rebuild summary.
...
- Performance budget: no AI/external calls, at most 22 DB queries, seeded response p95 below 250ms. `tests/unit/keyword-command-center-perf.test.ts` blocks the full assembler, pins one read per normalized projection source, and records first-paint/interaction p50/p95.

Local deterministic evidence (2026-07-13, 400-page seeded fixture, 30 warmed samples): interaction p50 `49.60ms → 38.50ms` (-22%), p95 `65.32ms → 44.76ms` (-31%). The audited SQL-source fanout falls from approximately 32 reads to a budgeted maximum of 22 (31% reduction); the deterministic test separately pins every normalized projection source to exactly one read.
