...
- The `/grouped` endpoint is the deliberately unpaginated skinny read for Pages, Clusters, and Lifecycle. It applies the same filter/search/sort scope as `/rows`, projects every matching KCC candidate from one source snapshot, and returns complete group membership plus server-owned keyword, click, impression, and average-position rollups. The UI must render these values directly; it must not regroup a paginated `/rows` response or recompute rollups.
...
- **`local_candidates` advanced filter.** `/rows?filter=local_candidates` uses the KCC source snapshot's already-loaded workspace, active markets, tracked keywords, table-backed content gaps, lite page assignments, feedback, and posture to build the same canonical candidates without re-reading the full local-candidate input set. The lite page projection includes `serp_features` because local-pack evidence participates in canonical candidate eligibility. The path prioritizes unselected candidates before selected candidates, applies `LOCAL_CANDIDATE_ROW_LIMIT`, then finalizes only that capped candidate slice through the normal row finalizer. The uncapped keyword-universe path (`UNIVERSE_SAFETY_CEILING`, unconditional since the `keyword-universe-full` flag was retired in flag-sunset Wave 2b) does NOT lift this cap. `/initial?filter=local_candidates` is rejected so first paint cannot enter the advanced filter indirectly; the client falls back to the normal capped `/rows` read for that filter. CI deep-pins loaded-snapshot candidate count and evidence against `buildLocalSeoKeywordCandidates()` and hard-gates the production filter at the registered 22-statement budget with an active local market.
...
- Summary rank KPIs use adjacent 28-day observation-date windows anchored to the latest stored rank snapshot: current `[latest - 27 days, latest]`, comparison `[latest - 55 days, latest - 28 days]`, all bounds inclusive. Because each stored snapshot already represents a rolling 28-day Search Console observation, each period selects its latest snapshot rather than summing overlapping captures. Average position is impression-weighted, with a simple mean only when every query has zero impressions; positive position delta means improvement (`comparison - current`). No snapshots means null window dates, values, and deltas. A current snapshot without a prior-window snapshot preserves the current average but leaves comparison values and all deltas null. The UI renders null as unavailable, never zero.
- Performance budget: no AI/external calls, at most 22 executed SQL statements per `/initial` or `/rows` request, and a manual external-log p95 target below 250ms. CI hard-gates executed SQL through the test-only better-sqlite3 execution counter and pins one read per normalized projection source. Seeded wall-clock p50/p95 is recorded as advisory evidence because shared-runner contention makes elapsed-time assertions nondeterministic. The current persisted slow-request telemetry only records requests at or above 1500ms, so it cannot own or claim enforcement of the 250ms target; evaluate that target from a representative external-log sample until finer KCC telemetry is intentionally added.
