...
| `ds-icon-discipline` | `from 'lucide-react'` OR `\bfa-[a-z-]+\b` string literal OR emoji codepoint class in rebuild `*.tsx` | `// icon-ok` | Kit rule: Font Awesome Sharp Regular via `Icon` + `ICON_NAMES` only; no emoji, no Lucide, no hard-coded `fa-` classes |
