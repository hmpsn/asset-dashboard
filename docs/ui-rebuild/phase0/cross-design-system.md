...
3. **Icon system: lucide-react (HEAD) or Font Awesome Sharp Regular (kit)?** FA implies a paid kit script (`kit.fontawesome.com`), an external runtime dependency, and a migration of hundreds of lucide call sites; lucide implies restyling the kit's Icon/ICON_NAMES contract onto lucide names. The kit's own leaf `.d.ts` files assume Lucide-style component icons, so lucide is the lower-risk reading — but the readme says FA is "the icon system." Owner call. **RESOLVED 2026-07-03: owner chose Font Awesome Sharp Regular (self-hosted, no CDN script); `<Icon name>`; lucide kept as the `<Icon as>` migration bridge, incremental. See PHASE_D_DECISIONS D5 + PR #1475.**
