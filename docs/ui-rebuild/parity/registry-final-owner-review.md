...
| Route home | Surface | Status | Remaining difference or accepted exception |
|---|---|---|---|
| `seo-strategy` | Insights Engine | `owner-approved` | Preserve approved V4–V6 exceptions. |
| `brand` | Brand & AI | `owner-approved` | Preserve approved V5–V7 production/data/design-system exceptions. |
...
| `page-intelligence` | Page Intelligence | `owner-approved` | Standalone prototype-led Research workbench implemented under final `ODP-012 B`; flag-off legacy rendering remains. Post-approval commits `3b7f4343f` and `e4b12beb7` hardened mobile usability/focus, deep-link refresh resolution, and exact-once action/handoff coverage without altering the approval record. |
...
| `revenue` | Revenue | `owner-approved` | The local account is truthfully empty while the prototype is populated. |
| `ai-usage` | AI Usage | `owner-approved` | Existing provider/cost/credit operations remain additive. |
| `features` | Features | `owner-approved` | Existing sales-library grouping and actions remain additive. |
| `prospect` | Prospects | `owner-approved` | Existing report generation/history remains additive. |
| `outcomes-overview` | Outcomes Book | `owner-approved` | Server-owned evidence and `GO-004` authority replace prototype-computed portfolio claims. |
| `outcomes` | Outcome Dashboard | `owner-approved` | No direct prototype exists; the retained dashboard is the accepted Global Ops exception until a receiving-home project is specced. |
| `diagnostics` | Diagnostics | `owner-approved` | The list/run-history home is a production routing exception; stage/export remains unavailable under `GO-003`. |
| `requests` | Requests | `owner-approved` | The four-mode hybrid remains under `GO-001`; Promote-to-signal remains unavailable under `GO-002`. |

## Final Paired Evidence

New registry-closeout captures are under:

- Prototype: `/tmp/asset-dashboard-codex-visual-parity/registry-final/prototype/`
- Rebuilt: `/tmp/asset-dashboard-codex-visual-parity/registry-final/rebuilt/`
- Visual review sheets: `/tmp/asset-dashboard-codex-visual-parity/registry-final/contact-sheets/`

Every file in the registry-closeout set was measured from the file itself, not trusted by filename. Mislabelled historical artifacts were excluded from strict evidence.

High-value state coverage added during closeout:

| Surface | Prototype evidence added | Rebuilt evidence added |
|---|---|---|
| Content Pipeline | Board, Brief, Draft, Review, capacity, Calendar, Published, and Content Health at both viewports | Brief, Draft, Review, capacity, Calendar, Published, and Content Health at 1600; existing reviewed 1440 evidence retained |
| SEO Editor | Default, selected toolbar, Manual, static/CMS Research, and 600/860px Research widths at both viewports | Selected toolbar, Manual, static/CMS Research, and 600/860px Research at 1600; existing reviewed 1440 evidence retained |
| Search & Traffic | Search, Site Traffic, and Annotations at 1600, with strict 1440 counterparts | Site Traffic and Annotations at 1600; existing reviewed 1440 evidence retained |
| Repair cluster | Asset repair selection, Performance Page Speed, and Site Audit History at both prototype viewports | Matching 1600 states plus existing reviewed 1440 evidence; Site Audit compatibility support captured at 1600 |
| Schema / Links / Keywords | Schema Guide, all four Links modes, all five Keyword lenses, and Keyword detail at 1600 | Matching 1600 interiors plus existing reviewed 1440 evidence |
| Local Presence | Rank/profile, Reviews/replies, and market setup at 1600 | Reviews/replies and market setup at 1600 plus existing reviewed 1440 evidence |
| Global Ops | Business subviews at both viewports; Roadmap Backlog at both; Outcomes, Diagnostics, and Requests reference states at 1600 | All Requests modes at 1600 plus the previously reviewed fixed-viewport Global Ops archive |
| Page Intelligence | Prototype live Research detail/pinned-drawer composition used for the approved standalone adaptation | Rebuilt 1440 empty/detail, 1600 empty/detail, and 390 empty/detail under `/tmp/asset-dashboard-codex-visual-parity/page-intelligence/` |

The source-led Page Rewriter pair remains under `/tmp/asset-dashboard-codex-visual-parity/batch9/page-rewriter/`. Its loaded, empty, picker, export, Focus, and mobile states passed fresh Sol review at both required desktop sizes.

## Explicit Data-State Mismatches
...
These states were reviewed for containment, typography, exact-once reachability, URL initialization, and capability preservation. They are not presented as pixel-matched prototype states.

## Owner-Approved Focus Decision
...
Page Intelligence is now mounted through `REBUILT_SURFACES` as a standalone, Joshua-approved permanent master/detail adaptation of the prototype's live Research detail/pinned-drawer composition; the prototype's retained `workMode` toggle is a no-op/dead path, and the flag-off legacy receiver remains intact. It preserves every established analysis, edit, tracking, provider, Architecture, Guide, and cross-surface capability and adds validated `?tab=` and `?page=` initialization. Post-approval commit `3b7f4343f` hardened the 390px list→detail→back usability floor, cold deep-link restoration, and exact-once actions/handoffs; `e4b12beb7` added Back/origin focus continuity and cached-background-refresh identity resolution. Neither changes the approved desktop composition or owner-approval history. Evidence under `/tmp/asset-dashboard-codex-visual-parity/page-intelligence/` now includes rebuilt 1440×900 empty/detail, 1600×1000 empty/detail, and 390px empty/detail captures. Joshua explicitly approved the receiving-home bundle on 2026-07-11; rendered review and automated gates remain supporting evidence only.
...
Current verification is intentionally not described as a clean release gate. Focused suites, hooks lint, typecheck, production build, PR checks, AI quality/reliability reports, and live fixed-viewport smoke support the repair wave. The clean current all-project suite passed 2,061 files and 28,817 tests, with one intentional skip and three todos. Keywords now passes its entry budget. Bundle-budget verification remains red for CSS 37.5 KiB versus 36.4 KiB, Page Rewriter 8.7 KiB versus 8.1 KiB, and aggregate 1.75 MiB versus 1.72 MiB; 71 new assets are warning-only. The seven owner decisions and an explicit bundle disposition remain required.
