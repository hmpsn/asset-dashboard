...
2. Insights Engine single-spine correction
   - Status: `ODP-001 A` accepted; contract, current-state tests, smoke, and trust-spine preview slice complete; implementation queued.
   - Reason: The prototype is a single strategy spine, while the rebuilt surface is split across visible top-level lenses. This is a behavior/IA mismatch, but changing it affects a high-impact operator workflow.
   - Deliverable: collapse visible top-level lenses into one prototype spine while preserving `?lens=` as section-open/focus state. Current safe pre-work covers no internal labels, exact-once Add Recommendation modal, exact-once move drawer, send disabled/docked behavior, legacy tab redirects, responsive mobile header/verdict fixes, the prototype client trust-spine preview inside the current spine lens, and desktop/mobile/deep-link/open-state smoke evidence. Latest preview smoke captured `/tmp/asset-dashboard-codex-parity-captures/engine-trust-spine-preview-scrolled-desktop.png` with no overflow or internal labels; local preview noise was limited to Vite WebSocket warnings and one route-change-aborted intelligence request.
