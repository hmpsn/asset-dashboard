...
All 26 mounted rebuilt route homes are now `owner-approved`. Joshua first approved Insights Engine (`ODP-001-V1` through `V6`) and Brand & AI (`V1` through `V7`), then approved the remaining 24 route homes as one registry batch with every documented production/data/backend exception and the Page Rewriter 62px Focus rail retained. Final evidence lives in the surface archives and `/tmp/asset-dashboard-codex-visual-parity/registry-final/`; the consolidated decision record is `registry-final-owner-review.md`.
