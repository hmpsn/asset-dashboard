...
const durableIdSchema = z.string().trim().min(1).max(200);
...
const sourceRevisionSchema = z.object({
  matrix_revision: z.number().int().nonnegative(),
  template_revision: z.number().int().nonnegative(),
  cell_revision: z.number().int().nonnegative(),
});
...
export const getContentMatrixInputSchema = z.object({
  workspace_id: workspaceIdSchema,
  matrix_id: durableIdSchema,
  cursor: cursorSchema.optional()
    .describe('Opaque cell cursor bound to the matrix ID and matrix revision.'),
  limit: z.number().int().min(1).max(MATRIX_READ_LIMITS.maxPageSize).optional()
