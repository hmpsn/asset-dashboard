  ...
  try {
    ...
    if (strategyMode === 'incremental') {
      ...
      if (pagesToAnalyze.length === 0) {
        ...
        clearKeepalive(); // prevent setInterval leak on early exit
        ...
        responseSent = true;
        return { strategy: ws.keywordStrategy ?? null, upToDate: true, freshPageCount: pagesToPreserve.length };
      }
    }
...
- Return ONLY valid JSON, no markdown`;
    ...
    if (!masterData) {
      ...
      throw new KeywordStrategyGenerationError(500, { error: errMsg, raw: masterRaw.slice(0, 500) });
    }
