/**
 * work_order dual-write mirror.
 *
 * At the work_order CREATE + UPDATE seams (`createWorkOrder` after insert, and `updateWorkOrder`
 * after a status change — both in `server/work-orders.ts`), mirror the `WorkOrder` into the
 * unified `client_deliverable` model via the registered `work_order` adapter + `upsertDeliverable`.
 *
 ...
 * logged and swallowed (the order is already persisted by the time the mirror runs).
 *
 ...
 * order) and the update seam (after a status change), so the order deliverable reflects lifecycle
 * progress. Idempotent on `work_order:<id>` — a re-mirror UPDATEs the same row. Returns the
 * mirrored deliverable, or null when the mirror was skipped/failed. Never throws — the live legacy
 * create/update must not be affected.
 */
