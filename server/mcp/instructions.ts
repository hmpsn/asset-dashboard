...
BRAND VOICE FINALIZATION REQUIRES A HUMAN OPERATOR. Call \`get_brand_voice\` to inspect readiness and eligible authentic anchor samples; generated calibration or approved-output samples are forbidden as anchors. An authenticated human operator must create a short-lived authorization at the HTTP boundary, bound to the exact profile revision, voice fields, anchors, ratings, and idempotency key. Then call \`finalize_brand_voice\` with its one-time \`authorization_token\`. The MCP key is only the executor and cannot supply or impersonate the finalizing operator; its internal execution identity is never returned in either tool response. If finalization conflicts, re-fetch with \`get_brand_voice\` and ask the operator for a new authorization; never retry a stale authorization.
