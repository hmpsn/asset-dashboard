...
router.get('/api/audit-schedules/:workspaceId', requireWorkspaceAccess('workspaceId'), (req, res) => {
  const schedule = getSchedule(req.params.workspaceId);
  if (!schedule) return res.status(404).json({ error: 'No schedule found' });
  res.json(schedule);
});

router.put('/api/audit-schedules/:workspaceId', requireWorkspaceAccess('workspaceId'), (req, res) => {
...
router.delete('/api/audit-schedules/:workspaceId', requireWorkspaceAccess('workspaceId'), (req, res) => {
