...
export const DEPRECATION_REGISTRY: DeprecationEntry[] = [
  ...
  },
  {
    ...
    replacement: 'Enable the keyword-value-scoring flag to swap the Hub opportunity sort from the crude volume×ease score to value-first scoring.',
    requiresHumanVerification: false,
    notes: 'The value-first scorer ships behind a default-OFF flag so old-vs-new is comparable on staging; the flag-OFF path is byte-identical to today. Survives the Keyword Hub cutover with its own removal condition.',
    contracts: [
      {
        kind: 'visibility-gate',
        description: 'The opportunity sort uses computeKeywordValueScore only when keyword-value-scoring is ON; OFF keeps computeOpportunityScore (byte-identical).',
        evidence: 'shared/types/feature-flags.ts (keyword-value-scoring default false in FEATURE_FLAGS)',
        testEvidence: 'tests/unit/feature-flags-keyword-hub.test.ts (keyword-value-scoring survives, defaults OFF)',
      },
    ],
  },
  {
