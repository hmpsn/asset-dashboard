...
function SummaryTiles({ state }: { state: KeywordsSurfaceVisualState }) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      <MetricTile label="Total" value={state === 'empty' ? 0 : summary.counts.total} sub="All" accent="var(--blue)" />
      <MetricTile label="In Strategy" value={state === 'empty' ? 0 : summary.counts.inStrategy} accent="var(--teal)" />
      <MetricTile label="Tracked" value={state === 'empty' ? 0 : summary.counts.tracked} accent="var(--blue)" />
      <MetricTile label="Needs Review" value={state === 'empty' ? 0 : summary.counts.needsReview} accent="var(--amber)" />
      <MetricTile label="Monthly Value" value={state === 'empty' ? 'No source' : '$1,234'} sub="Display-only" accent="var(--emerald)" />
    </div>
  );
}
      ...
      <div className="mx-auto flex max-w-[1180px] flex-col gap-5">
        ...
        <Toolbar label="Keyword view controls" className="w-full">
          <LensSwitcher
            ...
            size="sm"
          />
          <SearchField value="" onChange={() => {}} placeholder="Search keywords" className="min-w-[220px] flex-1" />
          <ToolbarSpacer />
          <span className="t-caption text-[var(--brand-text-muted)]">ws-visual</span>
        </Toolbar>
