...
describe('canonical MCP tool registry', () => {
  it('is the sole 19-family, 110-tool discovery source with exact global tools', () => {
    const definitions = listMcpToolDefinitions();
    expect(definitions).toHaveLength(110);
  ...
  it('records each declared workspace field and uses json_v1 for every production tool', () => {
    ...
    expect(entries.filter(entry => entry.errorContract === 'json_v1')).toHaveLength(110);
  });
