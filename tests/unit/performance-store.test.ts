...
describe('performance-store', () => {
  ...
  it('saves and retrieves each performance snapshot subtype independently', () => {
    ...
    savePageSpeed(SITE_ID, { score: 91 });
    ...
    expect(getPageSpeed(SITE_ID)?.result).toEqual({ score: 91 });
  ...
  it('overwrites snapshots with the same subtype and site id', () => {
    savePageSpeed(SITE_ID, { score: 70 });
    savePageSpeed(SITE_ID, { score: 95 });

    expect(getPageSpeed(SITE_ID)?.result).toEqual({ score: 95 });
  });
