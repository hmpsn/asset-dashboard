...
const loadSource = async (modulePath: string) => {
  ...
  return fs.readFileSync( // readFile-ok — wiring guard: asserts slice assemblers reference the required modules (cannibalization-detection, content-decay, site-architecture, page-keywords, schema-validator) so mini-builder functions receive correct data.
    path.resolve(import.meta.dirname, modulePath),
    'utf-8',
  );
};
...
describe('buildBriefIntelligenceBlock data covered by slice assemblers', () => {
  it('contentPipeline assembler references cannibalization-detection module AND uses cannibalizationWarnings', async () => {
    const src = await loadSource('../server/intelligence/content-pipeline-slice.ts');
...
describe('buildPlanContextForPage data covered by slice assemblers', () => {
  it('seoContext references ctx.strategy or strategy data', async () => {
    const src = await loadSource('../server/intelligence/seo-context-slice.ts');
  ...
  it('pageProfile assembler references site-architecture AND page-keywords modules', async () => {
    const src = await loadSource('../server/intelligence/page-profile-slice.ts');
  ...
  it('siteHealth assembler references schema-validator AND schemaErrors', async () => {
    const src = await loadSource('../server/intelligence/site-health-slice.ts');
