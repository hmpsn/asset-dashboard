...
describe('PATCH /api/roadmap/item/:id — Zod validation, sprint scoping, field whitelist', () => {
  ...
  let originalStatus: 'pending' | 'in_progress' | 'done' | 'deferred' | 'closed' = 'pending';

  beforeAll(async () => {
    const res = await api('/api/roadmap');
    const body = await res.json() as { sprints: Array<{ id: string; items: Array<{ id: number | string; status: 'pending' | 'in_progress' | 'done' | 'deferred' | 'closed' }> }> };
