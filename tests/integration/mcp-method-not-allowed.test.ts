 ...
 *
 * ROOT CAUSE this locks in: the MCP router only registered POST handlers. A
 * Streamable HTTP client opens GET on `/mcp` or `/mcp/operator` to establish the
 * optional server→client SSE notification stream (and sends DELETE to tear a
 * session down). With no GET/DELETE handler, those requests fell through to the SPA catch-all
...
describe('MCP endpoint — non-POST methods', () => {
  it.each(['/mcp', '/mcp/operator'])('%s rejects GET and DELETE without SPA fallthrough', async (path) => {
