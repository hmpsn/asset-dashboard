...
- ~~All OpenAI text models upgraded to current GPT-5 family~~: ✅ Refreshed (May 6, 2026) — primary structured/analytical paths use `gpt-5.5`, cost-sensitive paths use `gpt-5.4-mini`, and trivial classification/summary paths use `gpt-5.4-nano`. Brand name context is injected into all AI prompts that generate client-facing copy.
- ~~All SEO rewrites → Claude primary~~: ✅ Shipped (March 24, 2026) — `/api/webflow/seo-rewrite`, `/api/webflow/seo-bulk-fix`, and `/api/webflow/seo-bulk-rewrite` now use `callCreativeAI()` (Claude Sonnet primary, `gpt-5.5` fallback) for richer, more natural title/meta description language.
...
- ~~Claude/GPT hybrid model~~: ✅ Refreshed (May 6, 2026) — Claude (`claude-sonnet-4-6`) for creative prose (intro/sections/conclusion), with `gpt-5.5` / `gpt-5.4-mini` for OpenAI structured tasks (unification, SEO meta, briefs). Auto-fallback to GPT if no Anthropic key.
