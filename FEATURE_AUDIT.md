...
- **Cannibalization dedupe-vs-insight (two sites).** Cannibalization is ALSO an `InsightType`. **Rec-gen site:** `getInsights(ws.id, 'cannibalization')` builds the set of URL-set keys covered by ACTIVE (unresolved) insights; an issue whose URL set matches an active insight does NOT mint a rec (cross-link instead). **Briefing site (G10):** `collectAllCandidates` runs a cross-source dedup after the collection loop — drops a `referenceType:'recommendation'` cannibalization candidate when a matching `referenceType:'analytics_insight'` cannibalization candidate covers the same URL set. Both key on `cannibalizationUrlSetKey()` (order-independent normalized slug join).
