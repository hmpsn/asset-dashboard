...
- **HowTo-shaped lists:** Ordered lists with a 2-step heuristic: (1) nearby `<h2>/<h3>` contains "how", "guide", "tutorial", "steps", "process" (case-insensitive), and (2) the list has ≥2 items. Emission: `HowTo` with `name` (heading text), `step` array with `text` per item. Failure mode: graceful — if heuristic fails, no emission, existing schema unchanged.
...
**New REQUIRED_BY_TYPE entries:** `VideoObject` (name, description, thumbnailUrl, uploadDate); `HowTo` (name, step).
