...
**Files:** `shared/types/analytics.ts` (competitorKeywordData), `server/semrush.ts` (normalizeProviderDate, backlinks methods), `server/dataForSeo.ts` (normalizeProviderDate, backlinks + keyword methods), `server/routes/strategy.ts` (competitor keyword limit + sort), `scripts/pr-check.ts` (2 regression rules), `tests/integration/seo-provider-competitorData.test.ts` (4 integration tests)
