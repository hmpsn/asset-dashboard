...
**Files:** `server/routes/webflow-seo.ts`; `server/routes/webflow-seo-suggestions.ts`; `server/routes/webflow-seo-page-tools.ts`; `server/routes/webflow-seo-rewrite.ts`; `server/webflow-seo-rewrite-utils.ts`; `server/webflow-seo-audit-bridges.ts`; `server/webflow-seo-bulk-accept-fixes-job.ts`; `server/webflow-seo-bulk-rewrite-job.ts`; `server/webflow-seo-bulk-analyze-job.ts`; `server/schemas/seo-bulk-jobs.ts`; `server/route-groups/webflow.ts`; `server/workspace-intelligence.ts`; `server/intelligence/page-elements-slice.ts`; `tests/unit/page-elements-slice.test.ts`; `tests/unit/webflow-seo-audit-bridges.test.ts`; `tests/unit/webflow-seo-bulk-accept-fixes-job.test.ts`; `tests/unit/webflow-seo-bulk-rewrite-job.test.ts`; `tests/unit/webflow-seo-bulk-analyze-job.test.ts`.
