...
**Agency value:** The strategy AI now selects from a verified closed candidate set with client signals baked in, validated by Zod with a retry, so malformed model output degrades to a deterministic non-empty strategy instead of an error or silent empty. Client-requested keywords are a hard guarantee, not a soft prompt.
