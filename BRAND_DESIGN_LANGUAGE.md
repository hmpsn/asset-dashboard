...
The first 27 route homes mounted through `REBUILT_SURFACES` have explicit owner-approved desktop visual parity or an owner-approved exception. The original 26-route batch was approved on 2026-07-10; Page Intelligence became the 27th direct mount and Joshua explicitly approved it with the Content Performance receiving-home bundle on 2026-07-11. W3.1 adds AI Visibility as the 28th direct mount under an embedded dedicated-home default that remains awaiting owner review. W4.1b adds a separately registered book-level Command Center at `/`, also awaiting owner visual review; W4.1c gives that workspace-less home one registry-owned “All workspaces” identity across rebuilt sidebar, Command Palette, and breadcrumbs. Neither phase changes the 28-route `Page` census or the workspace Cockpit. Content Performance is an owner-approved flag-on fold into Pipeline Published, not a direct mount; its canonical route id and legacy receiver remain available flag-off. The per-surface contracts in `docs/ui-rebuild/parity/` remain the authority for exact geometry and exceptions; this section records the shared baseline that emerged from the source-led pass.
...
| **Icon** | `ui/Icon.tsx` | Inherits `currentColor`; consumer supplies hue via `className` (e.g. `text-teal-400`). Glyphs are decorative by default unless an accessible label is provided. | Font Awesome Sharp Regular semantic names (`name={...}` from `ICON_NAMES`) are the registry-only system of record; arbitrary glyph names and raw `fa`/`family` escape props are unsupported so the deterministic production CSS subset remains complete. `<Icon as={LucideIcon}>` remains a migration bridge. Strict size enum: `xs\|sm\|md\|lg\|xl\|2xl` (8/12/16/20/24/32px). Inline-flex `<span>` wrapper so it is safe inside `<p>`, `<li>`, flex rows. forwardRef. |
...
| **Tooltip** | `ui/overlay/Tooltip.tsx` | `bg-zinc-950 text-zinc-100 text-xs px-2 py-1 rounded shadow-lg` | Hover (500 ms delay) + focus (instant). `role="tooltip"`, `aria-describedby` on trigger. Portals to `document.body` — safe under `transform`/`filter` ancestors. Ordinary tooltips use `--z-tooltip`; a trigger inside the topmost canonical Modal/Drawer/ConfirmDialog uses `--z-modal-fullscreen`, resolved through the shared overlay-stack machinery, so contextual help remains above its owning overlay without raising unrelated page tooltips. |
...
The detail implementation may be deferred until selection or a `?q=` deep link requests it, but the loading boundary must use the same canonical Drawer geometry and focus/scroll semantics as the loaded interior. Pointer-opened rows restore focus after Close/Escape; nested row controls remain independent. This is a performance boundary, not a new visual primitive or exception.
...
| Row rank trend | Blue `Sparkline`; emerald improvement pill, red decline pill, zinc stable pill; muted unavailable/establishing copy | The line is read-only data, while the delta pill communicates measured direction without fabricating missing history |
...
| 2026-07-11 | **Receiving-home approval and post-parity interaction/performance closeout**: no new token or hue. Joshua explicitly approved the Page Intelligence direct mount and folded Content Performance receiver, bringing the mounted registry to 27. Canonical Modal/Drawer/ConfirmDialog layers coordinate topmost focus, keyboard, backdrop, and reference-counted scroll lock; an overlay-owned Tooltip now rises through that same stack to `--z-modal-fullscreen`, while ordinary page tooltips retain `--z-tooltip`. Contextual loading fallbacks preserve the approved composition while Admin Chat, Content Pipeline, and the Keywords detail interior defer closed content, Search & Traffic scopes provider reads by active lens, and text-only Markdown consumers no longer inherit chart helpers. Tailwind's explicit production source scope changes generated CSS only, not token or utility authority. Page Intelligence adds a contained 390px list-to-detail/back regression floor with focus transfer to Back and restoration to the originating row, while keeping the approved desktop canvas unchanged. |
| 2026-07-11 | **Post-parity bundle closeout**: no new token, hue, typography role, or visual primitive. Production Font Awesome base CSS is generated deterministically from the 52 `ICON_NAMES` glyphs while the full licensed stylesheet remains the read-only generator source; `Icon.name` is registry-only and the Lucide migration bridge remains. Scanner-proven dead compatibility CSS and the production-only DS harness chunk were removed without changing live utility candidates or the development route. Page Rewriter preserves its approved composition and export controls while the heavy DOCX implementation loads on demand from a click-time editor snapshot; clipboard and print entry remain synchronous. |
| 2026-07-12 | **Post-parity decision composition closeout**: no new token, hue, typography role, or shared primitive. Insights Engine restores capability history and refresh truth inside the approved spine; Cockpit and Pipeline Published add compact secondary evidence bands using existing metric/card roles; the rebuilt shell uses the existing `AppShell.footer` slot for connection health and exposes the canonical Command Palette through visible chrome. Loading, error, stale, unknown-tier, and unavailable states remain neutral/amber truth signals rather than success-colored empty data. |
| 2026-07-17 | **Rebuilt Keywords inline rank trends**: no new token, hue, typography role, or shared primitive. The full Rankings DataTable reuses the blue `Sparkline` data primitive, emerald/red/zinc `Badge` direction semantics, and muted contextual copy for unavailable or insufficient snapshots. The server owns the 7-day delta; the client never infers a stable zero from missing evidence. |
| 2026-07-17 | **Rebuilt Keywords summary/grouped read models**: no new token, hue, typography role, or primitive. Avg. position and Position change reuse blue `MetricTile`s because both are read-only metrics; unavailable server evidence uses the muted em dash plus “Unavailable.” Pages/Clusters/Lifecycle keep the existing `GroupBlock`/`BoardColumn` compositions but now display complete server-owned membership and rollups, so the temporary amber truncation banner is removed. |
| 2026-07-17 | **Book-level Command Center navigation closure**: no new token, hue, typography role, or primitive. The rebuilt sidebar uses a dedicated teal “All workspaces” zone for the registry-owned Command Center entry and keeps workspace Cockpit distinct beneath it. Active/current state is route-truthful at `/`; the Command Palette and breadcrumbs reuse the same label and destination identity. Flag-OFF navigation remains unchanged. |
...
The standalone flag-on Page Intelligence surface adopts the prototype SEO Editor Research geometry: a compact 22px header band, dense controls, a `minmax(360px, 42%)` page rail, independent rail/detail scrolling, a deliberate empty-selection state, sticky target context, and a sticky footer containing only real production handoffs. At the 390px usability floor, the rail becomes a list-to-detail flow: selecting a page replaces the list and transfers focus to `Back to pages`; Back restores both the list and the originating row focus; footer actions wrap inside the workbench. The approved desktop geometry remains unchanged at `md` and above. It reuses established typography roles, DS primitives, Font Awesome semantic icon keys, surface/border tokens, teal actions, blue data, and canonical score colors. This is a surface composition, not a new primitive or token rule; flag-off legacy presentation remains unchanged.
...
## Brand Voice review and approval (2026-07-15)

The admin Brand Voice surface leads with `Review & approve` and an always-visible `InlineBanner`: amber while the current profile requires human approval, emerald only after an immutable voice version is locked. The approval checklist uses `Badge` status tones (emerald ready, amber required), `Disclosure` for exact DNA/guardrail/context review, shared `Checkbox` rows for authentic anchors, and `ConfirmDialog` before an individual MCP-proposed sample gains operator-attested provenance. When multiple proposals are present, a teal `Approve all N proposals` action appears above the complete untruncated visible set and is revision-bound to those exact sample IDs.

The Brand & AI top bar also exposes one pending-count action. Its canonical `Modal size="workflow"` shows every pending chat proposal and draft brand deliverable in full, without pagination or truncation, before offering `Approve all N changes`. Voice finalization is displayed in the same review queue but remains a separate explicit lock action because it establishes downstream generation authority. The primary teal action locks the voice in the platform. A secondary action issues the existing one-time MCP authorization code; its blue information banner communicates expiry and exact-revision binding. No new token, hue, card primitive, or reusable approval credential is introduced.
