...
// ── Payload field display names ─────────────────────────────────────────────

/** Canonical display names for approval-item payload fields shown to clients. */
...
/**
 * Single authority for payload-field display names in client views. Raw field
 * identifiers ("seoTitle") must never reach the DOM — unknown fields fall back to a
 * generic camelCase / snake_case / kebab-case → Title Case split. Returns null for
 * empty input so callers can apply their own default label.
 */
export function humanizeFieldLabel(field: string | null | undefined): string | null {
