...
export function SearchContextBand({ workspaceId, brandedDemand, feed, summary, loading, unavailableMessage }: SearchContextBandProps) {
  const demandMixUnavailableMessage = brandedDemand?.status === 'error'
    ? 'Split unavailable for this refresh.'
    : unavailableMessage ?? 'Brand tokens were not available for this workspace.';
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
        <GroupBlock
          ...
          stats={brandedDemand?.status === 'ready' ? [
            { label: 'branded', value: formatPercent(brandedDemand.branded?.sharePct), color: 'var(--blue)' },
            { label: 'non-brand', value: formatPercent(brandedDemand.nonBranded?.sharePct), color: 'var(--blue)' },
          ] : []}
        >
          {brandedDemand?.status === 'ready' ? (
            <div className="px-2 pb-2">
              <KeyValueRow label="Branded clicks" value={formatNumber(brandedDemand.branded?.clicks)} valueColor="var(--blue)" divider={false} />
