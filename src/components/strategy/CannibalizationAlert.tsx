...
const actionLabel = (action?: string) => {
  switch (action) {
    ...
    case 'noindex': return { label: 'Noindex', icon: EyeOff, color: 'text-[var(--brand-text)] bg-[var(--surface-3)]/30' };
    default: return null;
  }
};
        ...
        {items.slice(0, 10).map((item, i) => (
          <div key={i} className="px-3 py-2.5 bg-[var(--surface-3)]/40 rounded-[var(--radius-lg)] border border-[var(--brand-border)]">
            ...
            {item.action && (() => {
              ...
              return (
                <div className={`flex items-center gap-1 mt-1.5 px-1.5 py-0.5 rounded-[var(--radius-sm)] t-caption-sm font-medium ${a.color} w-fit`}>
