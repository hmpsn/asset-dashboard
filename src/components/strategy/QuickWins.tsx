...
export function QuickWins({ quickWins }: QuickWinsProps) {
  ...
  return (
    <div className="bg-[var(--surface-2)] border border-[var(--brand-border)] border-emerald-500/20 p-5 rounded-[var(--radius-signature)]">
      ...
      <div className="space-y-2">
        {quickWins.map((qw, i) => {
          const impactColor = qw.estimatedImpact === 'high' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : qw.estimatedImpact === 'medium' ? 'text-amber-400 bg-amber-500/10 border-amber-500/20' : 'text-[var(--brand-text)] bg-[var(--zinc-700)]/30 border-[var(--zinc-600)]/20';
          return (
