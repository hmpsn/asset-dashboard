...
interface CockpitSendPanelProps {
  ...
  disabled?: boolean;
  /**
   * Commit-button label. Optional — defaults to "Send to client" so every existing
   * consumer (StrategyCockpit, flag-OFF) stays byte-identical. The Issue cockpit passes
   * "Stage for issue" so the word "send" appears in exactly one place (the header commit).
   */
  commitLabel?: string;
}
...
export function CockpitSendPanel({ onSend, onCancel, disabled, commitLabel = 'Send to client' }: CockpitSendPanelProps) {
  ...
  return (
    <div className="rounded-[var(--radius-lg)] border border-[var(--brand-border)] bg-[var(--surface-3)] p-3 space-y-2">
      ...
      <textarea
        ...
        value={note}
        disabled={disabled}
        placeholder={`Add a note for the client (optional) — Enter to ${commitLabel.toLowerCase().startsWith('stage') ? 'stage' : 'send'}, Esc to cancel`}
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <Button size="sm" disabled={disabled} onClick={() => onSend(note.trim())}>{commitLabel}</Button>
      </div>
    </div>
  );
}
