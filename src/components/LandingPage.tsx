    ...
    <nav className="fixed top-0 w-full z-[var(--z-modal)] border-b border-[var(--brand-border)] bg-[var(--surface-1)]/80 backdrop-blur-xl">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <a href={SIGNUP_URL} className="px-4 py-2 rounded-[var(--radius-lg)] text-sm font-semibold bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white transition-all">
            Start Free
          </a>
        </div>
      </div>
  ...
  return (
    <section className="relative pt-32 pb-20 sm:pt-40 sm:pb-28 overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-6 text-center">
        ...
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[var(--brand-text-bright)] leading-[1.1] tracking-tight mb-6">
        ...
        <p className="text-lg sm:text-xl text-[var(--brand-text)] max-w-2xl mx-auto mb-10 leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <span className="text-sm text-[var(--brand-text-muted)]">No credit card required</span>
        </div>
            ...
            <div className="p-6 sm:p-10 space-y-4">
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={m.label} className="rounded-[var(--radius-xl)] bg-[var(--surface-3)]/50 border border-[var(--brand-border)]/50 p-4">
                    <div className="t-caption-sm text-[var(--brand-text-muted)] mb-1">{m.label}</div>
                    <div className="text-xl sm:text-2xl font-bold text-[var(--brand-text-bright)]">{m.value}</div>
                    <div className={`text-xs font-medium ${m.color} mt-1`}>{m.delta}</div>
                  </div>
                ))}
              </div>
              <div className="h-32 rounded-[var(--radius-xl)] bg-[var(--surface-3)]/30 border border-[var(--brand-border)]/30 flex items-end px-4 pb-4 gap-1">
    ...
    <section className="py-20 sm:py-28">
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-[var(--brand-text-bright)] mb-4">
            SEO shouldn&apos;t require a six-figure budget
          </h2>
          <p className="text-[var(--brand-text)] max-w-xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {problems.map((p, i) => (
            <div key={i} className="border border-[var(--brand-border)] bg-[var(--surface-2)]/50 p-6 hover:border-[var(--brand-border-hover)] transition-colors rounded-[var(--radius-signature)]">
              ...
              <h3 className="text-sm font-semibold text-[var(--brand-text-bright)] mb-2">{p.title}</h3>
              <p className="t-ui text-[var(--brand-text-muted)] leading-relaxed">{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
                ...
                <div className="space-y-2.5">
                  {p.features.map((f, j) => (
                    <div key={j} className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-[var(--brand-text)]">{f}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((s, i) => (
            <div key={i} className="text-center">
              ...
              <p className="text-sm text-[var(--brand-text)] leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
...
function Pricing() {
  const plans = [
    ...
    },
    {
      ...
      features: [
        ...
        'À la carte SEO fixes (meta, schema, redirects)',
        'ROI dashboard',
  ...
  return (
    <section id="pricing" className="py-20 sm:py-28 border-t border-[var(--brand-border)]/50">
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-14">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map(plan => (
            <div key={plan.id} className={`relative rounded-[var(--radius-xl)] border p-6 transition-all ${plan.highlighted ? 'bg-teal-500/[0.03] border-teal-500/30 ring-1 ring-teal-500/20' : 'bg-[var(--surface-2)] border-[var(--brand-border)] hover:border-[var(--brand-border-hover)]'}`}>
              ...
              <div className="pt-1">
                <h3 className={`text-lg font-bold ${plan.highlighted ? 'text-teal-300' : 'text-[var(--brand-text-bright)]'}`}>{plan.name}</h3>
                <div className="flex items-baseline gap-1 mt-2 mb-1">
                  <span className={`text-3xl font-bold ${plan.highlighted ? 'text-teal-300' : 'text-[var(--brand-text-bright)]'}`}>{plan.price}</span>
                  {plan.period && <span className="text-sm text-[var(--brand-text-muted)]">{plan.period}</span>}
                </div>
                <p className="t-caption-sm text-[var(--brand-text-muted)] mb-6">{plan.tagline}</p>
                <a href={SIGNUP_URL} className={`block w-full py-2.5 rounded-[var(--radius-lg)] text-sm font-semibold text-center transition-all ${plan.ctaStyle}`}>
                  {plan.cta}
                </a>
        ...
        <p className="text-center text-xs text-[var(--brand-text-dim)] mt-6">
    ...
    <section className="py-16 border-t border-[var(--brand-border)]">
      <div className="max-w-4xl mx-auto px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s, i) => (
            <div key={i} className="text-center">
              <div className="text-2xl sm:text-3xl font-bold text-[var(--brand-text-bright)]">{s.value}</div>
              <div className="text-xs text-[var(--brand-text-muted)] mt-1">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="py-20 sm:py-28 border-t border-[var(--brand-border)]">
      <div className="max-w-2xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-[var(--brand-text-bright)]">
            Questions? Answers.
          </h2>
        </div>
        <div>
...
function FinalCTA() {
  return (
    <section className="py-20 sm:py-28 border-t border-[var(--brand-border)]/50 relative overflow-hidden">
      ...
      <div className="relative max-w-3xl mx-auto px-6 text-center">
        ...
        <p className="text-[var(--brand-text)] max-w-lg mx-auto mb-8">
  ...
  return (
    <footer className="border-t border-[var(--brand-border)] py-10">
      <div className="max-w-5xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <p className="text-xs text-[var(--brand-text-dim)]">&copy; {new Date().getFullYear()} {STUDIO_NAME}. All rights reserved.</p>
      </div>
    </footer>
  );
}
