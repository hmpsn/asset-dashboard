...
function makeCells(): MatrixCell[] {
  ...
  const statuses: MatrixCell['status'][] = ['planned', 'keyword_validated', 'brief_generated', 'review', 'flagged', 'approved', 'draft', 'published'];
  let idx = 0;
...
export const MOCK_MATRIX: ContentMatrix = {
  ...
  stats: {
    ...
    reviewed: cells.filter(c => ['review', 'flagged', 'approved'].includes(c.status)).length,
