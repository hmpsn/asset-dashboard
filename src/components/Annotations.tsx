    ...
    <div className="space-y-4">
      ...
      {annotations.length > 0 ? (
          ...
          {annotations.sort((a, b) => b.date.localeCompare(a.date)).map(ann => (
            <div key={ann.id} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-zinc-900 border border-zinc-800 group hover:border-zinc-700 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                {ann.description && <span className="text-[10px] text-zinc-500 ml-2">{ann.description}</span>}
              </div>
              <button onClick={() => deleteAnnotation(ann.id)}
                className="opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-red-400 transition-all flex-shrink-0 p-1">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-[10px] text-zinc-600 mt-0.5">Add events like algorithm updates, site launches, or marketing campaigns</p>
        </div>
      )}
    </div>
  );
