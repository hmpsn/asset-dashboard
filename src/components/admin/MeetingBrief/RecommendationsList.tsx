      ...
      <div className="space-y-3">
        {items.map((rec, i) => (
          <div key={i} className="flex gap-3 p-3 rounded-[var(--radius-lg)] bg-[var(--surface-2)] border border-[var(--brand-border)]">
            <span className="shrink-0 mt-0.5 text-xs font-bold text-[var(--brand-text-muted)] w-5 text-center">
              {i + 1}
            </span>
            <div>
              <p className="text-sm font-medium text-[var(--brand-text-bright)]">{rec.action}</p>
              <p className="text-xs text-[var(--brand-text-muted)] mt-0.5">{rec.rationale}</p>
            </div>
          </div>
        ))}
      </div>
