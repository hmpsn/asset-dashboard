...
import {
  ...
  LogIn, LogOut, ChevronRight, Users, Unplug,
} from 'lucide-react';
...
export function SettingsPanel() {
  ...
  const disconnectGoogle = async () => {
    ...
    setGscSites([]);
    setWorkspaces(prev => prev.map(w => ({ ...w, gscPropertyUrl: undefined })));
  };
  ...
  const configured = workspaces.filter(w => w.gscPropertyUrl);
      ...
      {configured.length > 0 && (
        <section className="rounded-xl overflow-hidden" style={{ backgroundColor: 'var(--brand-bg-elevated)', border: '1px solid var(--brand-border)' }}>
          <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--brand-border)' }}>
            ...
            <p className="text-xs mt-0.5" style={{ color: 'var(--brand-text-muted)' }}>Share these links with clients. No login required — read-only access to search data and AI assistant.</p>
          </div>
          <div className="divide-y" style={{ borderColor: 'var(--brand-border)' }}>
            {configured.map(ws => (
              <div key={ws.id} className="px-5 py-4 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium" style={{ color: 'var(--brand-text-bright)' }}>{ws.name}</div>
                  <div className="text-[11px] text-zinc-500 truncate mt-0.5">{ws.gscPropertyUrl}</div>
                </div>
                <button
