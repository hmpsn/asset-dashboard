import {
  ChevronDown, ChevronUp, FileText, Copy, Download, Trash2, Search, X, ArrowUpDown,
  Clipboard,
} from 'lucide-react';
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                type="text"
                value={briefSearch}
                onChange={e => onSetBriefSearch(e.target.value)}
              ...
              />
              {briefSearch && (
                <button onClick={() => onSetBriefSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-zinc-400">
                  <X className="w-3 h-3" />
                </button>
              )}
            </div>
            <div className="flex items-center gap-1 text-[11px] text-zinc-500">
              ...
              <select value={briefSort} onChange={e => onSetBriefSort(e.target.value as 'date' | 'keyword' | 'difficulty')} className="bg-zinc-900 border border-zinc-800 rounded px-1.5 py-1 text-[11px] text-zinc-400 focus:outline-none cursor-pointer">
